...
class StreamPipeline:
    ...
    def encode_stream(self, in_stream, out_stream, file_size=None, checkpoint_path=None):
        ...
        if checkpoint_path:
            ...
            state = cp.load()
            if state:
                start_chunk = state.get('processed_chunks', 0)
                try:
                    in_stream.seek(start_chunk * chunk_size)
                except Exception:
                    pass 
